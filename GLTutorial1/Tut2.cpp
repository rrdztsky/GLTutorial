...
Tut2::Tut2()
{
	verts.emplace_back(0.0f, 0.0f, 0.0f); //template thingies forward arguments
	glGenVertexArrays(1, &vao);
	glBindVertexArray(vao);
	glGenBuffers(1, &vbo); //generate buffers at address of vbo (pointer)
	glBindBuffer(GL_ARRAY_BUFFER, vbo);
	glBufferData(GL_ARRAY_BUFFER, sizeof(Vec3), &verts[0], GL_STATIC_DRAW); //sends OpenGL the three floats.
	...
	void main()
	{
		FragColor = vec4(1.0, 1.0, 1.0, 1.0); //output a vec4 called FragColor, make whatever pixels it has whatever color I say
	}
	)";
...
{	
	...
	glDrawArrays(GL_POINTS, 0, 1); //draw points, start at index zero, draw one
	glDisableVertexAttribArray(0); //shuts down the attrib array
	glBindVertexArray(0);
}

void Tut2::reshape(int w, int h)
{
